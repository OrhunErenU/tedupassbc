 ...
 */
export async function addAttendanceByEmail(input: z.infer<typeof addAttendanceSchema>) {
  ...
  if (!email.endsWith(`@${ALLOWED_EMAIL_DOMAIN}`)) {
    throw new Error(`Sadece @${ALLOWED_EMAIL_DOMAIN} adresleri eklenebilir.`);
  }
  ...
  if (existing) throw new Error("Bu kişi zaten katılımcı listesinde.");
  ...
  return { name: attendee.name, email, isNew: !attendee.name };
}
...
export async function removeAttendance(attendanceId: string) {
  ...
  if (minted) throw new Error("Rozet zaten basılmış, katılım silinemez.");
